...
class Carla:
    ...
    def __init__(
        ...
        self.ego_spawn_point = self.map.get_spawn_points()
        ...
        self.lane_invation_transform = carla.Transform(carla.Location(0, 0, self.BEV_DISTANCE/2),carla.Rotation(0, 0, 0))
    ...
    def observation_space(self):
        return gym.spaces.Dict(
            {
                "image": spaces.Box(low=0, high=255, shape=(self.size[0], self.size[1], 3), dtype=np.uint8),
            }
        )
    ...
    def action_space(self):
        ...
        return gym.spaces.Box(low=np.array([0, -1]), high=np.array([1, 1]), dtype=np.float32)
    ...
    def reset(self):
        ...
        if count > 0:
            for spawn_point in self.spawn_points:
                ...
                if count <= 0:
                    break
        while count > 0:
            if self._try_spawn_random_vehicle_at(random.choice(self.spawn_points), number_of_wheels=[4]):
                count -= 1
        ...
        self.lane_invation_sensor = self.world.spawn_actor(self.lane_invation_sensor_bp, self.lane_invation_transform, attach_to=self.vehicle)
    ...
    def step(self, action):
        acc = action[0]
        steer = action[1]
        self.vehicle.apply_control(carla.VehicleControl(throttle=float(acc), steer=float(steer)))
        ...
        if len(self.collision_hist) != 0:
            ...
            reward = -20
        if len(self.lane_invation_list) != 0:
            reward = -5
            self.lane_invation_list.pop(0)
            print("lane crossed!")
        elif v_kmh < 20:
            reward = -1 * (20 - v_kmh) / 20
        else:
            reward = 1.0
    ...
    def get_observation(self):
        ...
        camera = resize(image, (self.size[0], self.size[1])) * 255

        # print("image shape !!!: %f", camera.shape)
        obs = {
...
class NormalizeActions:
    ...
    @property
    def action_space(self):
        low = np.where(self._mask, self._low, -np.ones_like(self._low))
        high = np.where(self._mask, self._high, np.ones_like(self._low))
