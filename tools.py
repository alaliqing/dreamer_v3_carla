...
class Logger:
  ...
  def image(self, name, value):
    self._image[name] = np.array(value)

  def images(self, name, value):
  ...
  def write(self, fps=False):
    ...
    for name, value in self._image.items():
      self._writer.add_image(name, value, self.step, dataformats='HWC')
    # for name, value in self._images.items():
    #   image_step = self.step//1000
    #   self._writer.add_images(name, value, image_step, dataformats='NHWC')
